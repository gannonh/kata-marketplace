...
```bash
PLAN_TEMPLATE_PATH=$(node "${CLAUDE_PLUGIN_ROOT}/skills/kata-plan-phase/scripts/kata-lib.cjs" resolve-template "plan-template.md")
...
```bash
GITHUB_ENABLED=$(node "${CLAUDE_PLUGIN_ROOT}/skills/kata-plan-phase/scripts/kata-lib.cjs" read-config "github.enabled" "false")
ISSUE_MODE=$(node "${CLAUDE_PLUGIN_ROOT}/skills/kata-plan-phase/scripts/kata-lib.cjs" read-config "github.issue_mode" "never")
```
