...
```bash
WORKTREE_OUTPUT=$(bash "${CLAUDE_PLUGIN_ROOT}/skills/kata-new-project/scripts/setup-worktrees.sh" 2>&1) || WORKTREE_FAILED=true
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/skills/kata-new-project/scripts/kata-lib.cjs" set-config "worktree.enabled" "false"
```
