...
4. **Execute waves**
   ...
   **IMPORTANT: The remaining post-wave steps are SEQUENTIAL. Do not run them in parallel.**
   ...
   - **Merge plan worktrees (if enabled) — do this FIRST:**
     ...
     if [ "$WORKTREE_ENABLED" = "true" ] && [ "$PR_WORKFLOW" = "true" ]; then
       for plan_num in $WAVE_PLAN_NUMBERS; do
         MERGE_OUTPUT=$(bash "${CLAUDE_PLUGIN_ROOT}/skills/kata-execute-phase/scripts/manage-worktree.sh" merge "$PHASE_NUM" "$plan_num" "$PHASE_BRANCH" "$WORKSPACE_PATH")
         eval "$MERGE_OUTPUT"
   ...
   - **Update GitHub issue checkboxes (if enabled):**
     ...
     bash "${CLAUDE_PLUGIN_ROOT}/skills/kata-execute-phase/scripts/update-issue-checkboxes.sh" "$PHASE" "$PHASE_DIR" $COMPLETED_PLANS_IN_WAVE
     ```
   ...
   - **Open Draft PR (first wave only, pr_workflow only):**
     ...
     if [ "$PR_WORKFLOW" = "true" ]; then
       ...
       if ! PR_OUTPUT=$(bash "${CLAUDE_PLUGIN_ROOT}/skills/kata-execute-phase/scripts/create-draft-pr.sh" "$PHASE_DIR" "$PHASE_BRANCH"); then
...
10.5. **Push and ensure PR exists (pr_workflow only)**
    ...
    if [ "$PR_WORKFLOW" = "true" ]; then
      ...
      if [ -z "$PR_NUMBER" ]; then
        ...
        PR_OUTPUT=$(bash "${CLAUDE_PLUGIN_ROOT}/skills/kata-execute-phase/scripts/create-draft-pr.sh" "$PHASE_DIR" "$PHASE_BRANCH" 2>&1) || true
        PR_NUMBER=$(gh pr list --head "$PHASE_BRANCH" --json number --jq '.[0].number' 2>/dev/null)
      fi
