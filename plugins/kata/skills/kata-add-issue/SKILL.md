...
```bash
GITHUB_ENABLED=$(node "${CLAUDE_PLUGIN_ROOT}/skills/kata-add-issue/scripts/kata-lib.cjs" read-config "github.enabled" "false")
```
...
```bash
COMMIT_PLANNING_DOCS=$(node "${CLAUDE_PLUGIN_ROOT}/skills/kata-add-issue/scripts/kata-lib.cjs" read-config "commit_docs" "true")
